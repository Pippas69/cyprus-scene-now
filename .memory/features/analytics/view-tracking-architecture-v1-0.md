...
- **Event results**: Navigate to `/event/:id` - tracked as event_view (source: search)
- **Offer results**: Navigate to `/offers` page - tracked as discount_view (source: search)
...
- `isAllowedEventViewSource()` - Checks current URL path + tab parameter (includes /business/:id)
- `isAllowedOfferViewSource()` - Checks current URL path + tab parameter (includes /business/:id)
...
- **Profile Views**: Views with `source = 'profile'` (from business profile pages)

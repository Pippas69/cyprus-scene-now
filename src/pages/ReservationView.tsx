...
import { Calendar, Clock, User, Copy, Users } from "lucide-react";
...
const ReservationView = () => {
  ...
  useEffect(() => {
    ...
    const fetchReservation = async () => {
      ...
      if (resData?.qr_code_token) {
        QRCode.toDataURL(resData.qr_code_token, {
          width: 512,
          margin: 2,
  ...
  if (!reservation) return <div className="min-h-screen flex items-center justify-center text-muted-foreground">{text.notFound}</div>;

  const locale = language === 'el' ? 'el-GR' : 'en-GB';
  const timeZone = 'Europe/Nicosia';
  const dateObj = reservation.preferred_time ? new Date(reservation.preferred_time) : 
                  reservation.event_start_at ? new Date(reservation.event_start_at) : null;

  const displayName = reservation.reservation_name || "-";
  const displayDate = dateObj ? new Intl.DateTimeFormat(locale, { timeZone, weekday: 'short', day: 'numeric', month: 'short' }).format(dateObj) : "-";
  const displayTime = dateObj ? new Intl.DateTimeFormat(locale, { timeZone, hour: '2-digit', minute: '2-digit', hour12: false }).format(dateObj) : "-";
  const isCheckedIn = !!reservation.checked_in_at;
  const title = reservation.event_title || text.tableReservation;
    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <div className="relative rounded-2xl overflow-hidden shadow-2xl w-full max-w-sm mx-auto">
        <div className="bg-gradient-to-br from-[#102b4a] to-[#1a3d5c] px-4 pt-5 pb-3 text-center">
          ...
          {reservation.business_name && <p className="text-white/70 text-[10px] mt-0.5">by {reservation.business_name}</p>}
        </div>
        ...
        <div className="bg-white dark:bg-white px-4 py-3">
          ...
          {isCheckedIn && (
            <div className="flex items-center justify-center gap-2 mb-3 p-2 bg-green-50 rounded-lg">
              ...
              <p className="text-sm font-semibold text-green-700">{text.checkedIn}</p>
            </div>
          )}
          ...
          {qrDataUrl && (
            <div className="flex flex-col items-center">
              <div className={`p-2 bg-white rounded-xl shadow-lg border-2 ${isCheckedIn ? 'border-green-300 opacity-50' : 'border-[#3ec3b7]'}`}>

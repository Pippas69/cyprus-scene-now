...
import { Key, Copy, Trash2, RefreshCw, Plus, Ticket, Users, CheckCircle, XCircle, Clock } from 'lucide-react';
...
const AdminBetaManagement = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      toast.error(language === 'el' ? 'Σφάλμα φόρτωσης' : 'Error loading data');
    } finally {
      setLoading(false);
    }
  };
  ...
  const toggleBetaMode = async () => {
    try {
      ...
      toast.success(newSettings.enabled 
        ? (language === 'el' ? 'Λειτουργία Beta ενεργοποιήθηκε' : 'Beta mode enabled')
        : (language === 'el' ? 'Λειτουργία Beta απενεργοποιήθηκε' : 'Beta mode disabled')
      );
    } catch (error) {
  ...
  const createCode = async () => {
    ...
    try {
      ...
      toast.success(language === 'el' ? `Κωδικός ${code} δημιουργήθηκε` : `Code ${code} created`);
      setNewCodeNote('');
      setNewCodeExpiry('');
      fetchData();
    } catch (error) {
      console.error('Error creating code:', error);
      toast.error(language === 'el' ? 'Σφάλμα δημιουργίας κωδικού' : 'Error creating code');
    } finally {
      setGenerating(false);
    }
  };
  ...
  const createBulkCodes = async () => {
    ...
    try {
      ...
      toast.success(language === 'el' ? `${bulkCount} κωδικοί δημιουργήθηκαν` : `${bulkCount} codes created`);
      fetchData();
    } catch (error) {
      console.error('Error creating bulk codes:', error);
      toast.error(language === 'el' ? 'Σφάλμα δημιουργίας κωδικών' : 'Error creating codes');
    } finally {
      setGenerating(false);
    }
  };
  ...
  const toggleCodeActive = async (id: string, currentActive: boolean) => {
    try {
      ...
      toast.success(!currentActive 
        ? (language === 'el' ? 'Κωδικός ενεργοποιήθηκε' : 'Code activated')
        : (language === 'el' ? 'Κωδικός απενεργοποιήθηκε' : 'Code deactivated')
      );
      fetchData();
    } catch (error) {
      toast.error(language === 'el' ? 'Σφάλμα ενημέρωσης' : 'Error updating');
  ...
  const deleteCode = async (id: string) => {
    ...
    try {
      ...
      toast.success(language === 'el' ? 'Κωδικός διαγράφηκε' : 'Code deleted');
      fetchData();
    } catch (error) {
      toast.error(language === 'el' ? 'Σφάλμα διαγραφής' : 'Error deleting');
    }
  };
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <Button onClick={createCode} disabled={generating} className="w-full">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <Button onClick={createBulkCodes} disabled={generating} variant="outline" className="w-full">
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              {language === 'el' ? 'Δεν βρέθηκαν κωδικοί' : 'No codes found'}
            </div>
          ) : (
            <Table>
              <TableHeader>
                ...
                {filteredCodes.map((code) => {
                  ...
                  return (
                    <TableRow key={code.id}>
                      ...
                      <TableCell>
                        {code.expires_at ? format(new Date(code.expires_at), 'dd/MM/yyyy') : '-'}
                      </TableCell>
                      <TableCell>

  ...
  const checkVerificationStatus = async () => {
    try {
      ...
      const [businessResult, profileResult] = await Promise.all([
        supabase
          .from("businesses")
          .select("id, verified, name, logo_url, cover_url, category, floor_plan_image_url")
          .eq("user_id", user.id)
          .maybeSingle(),
        supabase
          .from("profiles")
      ...
      setHasFloorPlan(!!businessResult.data.floor_plan_image_url);
  ...
  return (
    <>
      ...
      <div className="h-screen w-full flex overflow-hidden">
        <BusinessSidebar businessCategories={businessCategories} hasFloorPlan={hasFloorPlan} />

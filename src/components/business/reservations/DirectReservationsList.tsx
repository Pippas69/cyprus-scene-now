...
export const DirectReservationsList = ({ businessId, language, refreshNonce, onReservationCountChange, selectedEventId, selectedEventType, forceEventMode }: DirectReservationsListProps) => {
  ...
  useEffect(() => {
    ...
    // Check if business has floor plan enabled
    supabase.from('businesses').select('floor_plan_enabled').eq('id', businessId).single().then(({ data }) => {
      setHasFloorPlan(!!data?.floor_plan_enabled);
    });
  ...
  const EditableCell = ({ reservationId, field, displayValue, rawValue, inputType }: {reservationId: string;field: string;displayValue: string;rawValue: string;inputType?: string;}) => {
    ...
    return (
      <span
        className="cursor-pointer rounded px-1 py-0.5 transition-colors inline-flex items-center whitespace-nowrap -ml-2 group/edit relative pr-4"
        onClick={() => startEdit(reservationId, field, rawValue)}>
        
        {displayValue}
        <Edit2 className="h-3 w-3 text-muted-foreground opacity-0 group-hover/edit:opacity-100 transition-opacity absolute right-0 top-1/2 -translate-y-1/2" />
      </span>);

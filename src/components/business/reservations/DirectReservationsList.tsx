...
export const DirectReservationsList = ({ businessId, language, refreshNonce, onReservationCountChange, selectedEventId, selectedEventType, payAtDoor, forceEventMode, manualEntryOpen: externalManualEntryOpen, onManualEntryOpenChange, searchQuery, selectedDate }: DirectReservationsListProps) => {
  ...
  const EditableCell = ({ reservationId, field, displayValue, rawValue, inputType, inputClassName }: {reservationId: string;field: string;displayValue: string;rawValue: string;inputType?: string;inputClassName?: string;}) => {
    ...
    return (
      <span
        className="cursor-pointer rounded transition-colors inline-flex items-center gap-1 whitespace-nowrap -ml-2 group/edit py-0 text-sm mx-[5px] my-0 px-[2px]"
  ...
  const EventSeatingTypeEditCell = ({ reservationId, currentSeatingTypeId, seatingTypeName }: { reservationId: string; currentSeatingTypeId: string | null; seatingTypeName: string | null }) => {
    ...
    return (
      <Popover>
        <PopoverTrigger asChild>
          <span className="cursor-pointer rounded transition-colors inline-flex items-center gap-1 whitespace-nowrap group/edit text-sm text-foreground text-right my-0 px-0 py-[3px] mx-[33px]">
            {seatingTypeName || '—'}
            <Edit2 className="h-3 w-3 text-muted-foreground opacity-0 group-hover/edit:opacity-100 transition-opacity flex-shrink-0" />
          </span>
        </PopoverTrigger>
            ...
            <Table className="w-full min-w-[980px] table-fixed">
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-xs w-[11%] pl-2 pr-0 text-center">{t.seating}</TableHead>
              ...
              <TableBody>
                ...
                return (
                  <TableRow key={reservation.id} className="group hover:bg-transparent">
                        ...
                        <div className="flex items-center gap-1">
                          <div className="flex flex-col gap-0.5 min-w-0">
                            ...
                            {reservation.phone_number &&
                              <span className="-ml-1.5 mx-0 my-0 px-0 py-0 text-primary-foreground text-xs">
                            ...
                            if (isBottleRow && matchedTier) {
                              ...
                              return (
                                <div className="flex flex-col items-start">
                                  ...
                                  <span className="text-xs whitespace-nowrap px-0 my-[5px] text-primary-foreground">
                      ...
                      {/* 5. Κατάσταση */}
                      <TableCell className="align-top px-[20px]">

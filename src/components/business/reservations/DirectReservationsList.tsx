...
export const DirectReservationsList = ({ businessId, language, refreshNonce, onReservationCountChange, selectedEventId, selectedEventType, payAtDoor, forceEventMode, manualEntryOpen: externalManualEntryOpen, onManualEntryOpenChange, searchQuery, selectedDate }: DirectReservationsListProps) => {
  ...
  const EditableCell = ({ reservationId, field, displayValue, rawValue, inputType, inputClassName, className }: {reservationId: string;field: string;displayValue: string;rawValue: string;inputType?: string;inputClassName?: string;className?: string;}) => {
    ...
    return (
      <span
        className={`cursor-pointer rounded transition-colors inline-flex items-center gap-1 whitespace-nowrap -ml-2 group/edit text-sm my-0 py-0 mx-0 px-[12px] ${className || ''}`}

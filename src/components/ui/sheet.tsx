...
const sheetVariants = cva(
  cn(
    "fixed z-50 gap-4 bg-background/95 backdrop-blur-xl p-6 shadow-2xl",
...
const SheetContent = React.forwardRef<React.ElementRef<typeof SheetPrimitive.Content>, SheetContentProps>(
  ({ side = "right", className, children, showDragHandle, ...props }, ref) => {
      ...
      <SheetPortal>
        ...
        >
          ...
          <SheetPrimitive.Close className={cn(
            "absolute rounded-sm opacity-70 ring-offset-background transition-all",
            "hover:opacity-100 hover:bg-muted/50",
            "focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2",
            "disabled:pointer-events-none",
            "data-[state=open]:bg-secondary",
            side === "bottom" ? "right-4 top-4" : "right-4 top-4"
          )}>
            <X className="h-4 w-4" />
...
// Animated content wrapper for staggered children
interface SheetAnimatedContentProps {
  children: React.ReactNode;
  className?: string;
  staggerDelay?: number;
}

const SheetAnimatedContent = ({ children, className, staggerDelay = 50 }: SheetAnimatedContentProps) => {

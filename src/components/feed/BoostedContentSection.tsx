...
export const BoostedContentSection = ({
  ...
  const allContent: ContentItem[] = [...events.map(e => ({
    type: 'event' as const,
    data: e,
    ...
    // Use end_at for offers (soonest expiry first)
  ...
  return <div className="w-full">
      <ScrollArea className="w-full whitespace-nowrap">
        <div className="flex gap-3 pt-2 pr-2 pb-1">
          {allContent.map((item, index) => <motion.div key={`${item.type}-${item.data.id}`} initial={{
          opacity: 0,
          x: 20
        }} animate={{
          opacity: 1,
          ...
          delay: index * 0.03
              ...
              {item.type === 'event' ? <UnifiedEventCard event={item.data} language={language} isBoosted={true} size="boosted" /> : <OfferCard offer={item.data} t={t} language={language} />}
            </motion.div>)}
        </div>
...
const OfferCard = ({
  ...
  const getExpiryLabel = () => {
    ...
    const month = endDate.toLocaleDateString(language === "el" ? "el-GR" : "en-GB", {
      month: "long"
    });
    return language === "el" ? `${t.expiresOn} ${day} ${month}` : `${t.expiresOn} ${month} ${day}`;
  };
  ...
  return <>
      ...
      <div ref={cardRef as any} className="flex flex-col rounded-xl bg-card border border-border hover:border-primary/50 hover:shadow-lg transition-all duration-200 group min-w-[220px] max-w-[220px] sm:min-w-[240px] sm:max-w-[240px] overflow-visible">
        ...
        <div className="relative h-36 sm:h-40 overflow-visible">
          ...
          <Link to={`/business/${offer.business_id}`} className="absolute inset-0 overflow-hidden rounded-t-xl">
            {coverImage ? <img src={coverImage} alt={offer.businesses?.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy" /> : <div className="w-full h-full bg-gradient-to-br from-emerald-500/20 via-primary/10 to-secondary/20" />}
        ...
        <div className="flex-1 px-2 sm:px-3 pt-2 sm:pt-3 pb-1.5 sm:pb-2 flex flex-col justify-between min-h-0 gap-0.5">
          ...
          <div className="flex items-center gap-1 sm:gap-1.5 text-muted-foreground">
            <button onClick={handleMapClick} className="flex items-center text-muted-foreground hover:text-primary transition-colors flex-shrink-0" title={language === "el" ? "Δες στο χάρτη" : "View on map"}>
          ...
          <div className="flex items-center justify-between mt-0.5">
            {/* Discount badge on left */}
             <div className="flex items-center gap-1">
               {offer.percent_off && offer.percent_off > 0 && offer.discount_type !== "special_deal" && <Badge variant="default" className="text-[10px] sm:text-xs px-1.5 sm:px-2 py-0 h-5 sm:h-6">
                   -{offer.percent_off}%
                 </Badge>}

               {offer.discount_type === "special_deal" && offer.special_deal_text && <Popover>
            ...
            <div className="flex items-center gap-1 ml-auto">
              <Button variant="ghost" size="icon" className="h-5 w-5 sm:h-6 sm:w-6" onClick={handleShareClick} title={language === "el" ? "Κοινοποίηση" : "Share"}>
                <Share2 className="h-3 w-3 sm:h-3.5 sm:w-3.5" />
              </Button>
              <Button onClick={handleRedeemClick} size="sm" variant="default" className="text-[10px] sm:text-xs h-5 sm:h-6 px-2 sm:px-2.5">
                {t.redeem}
              </Button>
            </div>
          </div>
      ...
      {/* Purchase Dialog */}
      <OfferPurchaseDialog isOpen={isPurchaseOpen} onClose={() => setIsPurchaseOpen(false)} offer={{
      id: offer.id,
      title: offer.title,
      ...
      {/* Share Dialog */}
      <ShareOfferDialog open={isShareOpen} onOpenChange={setIsShareOpen} offer={{
      id: offer.id,
      title: offer.title,
      description: offer.description,
      percent_off: offer.percent_off,

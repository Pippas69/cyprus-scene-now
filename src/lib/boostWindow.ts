...
export const computeBoostWindow = (input: BoostWindowInput): BoostWindow | null => {
  ...
  if (durationMode === "hourly" && input.created_at) {
    const start = new Date(input.created_at).toISOString();
  ...
  if (input.start_date) {
    ...
    start = new Date(input.created_at).toISOString();
  }
  ...
  if (input.end_date) {
    ...
    end = addHoursISO(new Date(input.created_at).toISOString(), input.duration_hours);
  } else if (input.created_at) {
    end = new Date(input.created_at).toISOString();
  }

...
BEGIN
  ...
  IF p_user_id IS NOT NULL THEN
    -- Try to get email from auth.users for enrichment
    BEGIN
...
-- Backfill: enrich existing registered CRM guests with email from auth.users
UPDATE public.crm_guests cg
SET email = au.email, updated_at = now()
FROM auth.users au
WHERE cg.user_id = au.id
  AND cg.profile_type = 'registered'
  AND (cg.email IS NULL OR cg.email = '');

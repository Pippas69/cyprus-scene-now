...
CREATE OR REPLACE FUNCTION public.get_crm_guest_stats_v2(p_business_id uuid)
 RETURNS TABLE(guest_id uuid, total_visits bigint, total_spend_cents bigint, total_no_shows bigint, total_cancellations bigint, first_visit timestamp with time zone, last_visit timestamp with time zone, avg_party_size numeric, favorite_table text, total_reservations bigint)
 LANGUAGE sql
  ...
  SELECT
    ...
    COALESCE(
      tord.subtotal_cents / GREATEST(
        (SELECT COUNT(*) FROM public.tickets t2 WHERE t2.order_id = t.order_id AND COALESCE(t2.status::text, '') <> 'cancelled'),
        1
      ),

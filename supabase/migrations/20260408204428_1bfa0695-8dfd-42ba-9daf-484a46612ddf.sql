...
-- Ticket demographics (age + city from individual tickets)
...
-- Reservation demographics (age from guest_ages, city from guest_city - for booker only)
...
-- Profile demographics (age + city from user profiles)
-- For registered users: their own profile
-- For ghosts: inherit from booker (brought_by_user_id)
...
SELECT
  ...
  COALESCE(td.guest_age, rd.guest_age, pd.profile_age)::integer AS guest_age,
  COALESCE(td.guest_city, rd.guest_city, pd.profile_city) AS guest_city
FROM public.crm_guests cg

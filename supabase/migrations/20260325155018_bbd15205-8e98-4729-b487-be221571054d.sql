...
-- 1) Allow business owners to store real spend per reservation
ALTER TABLE public.reservations
ADD COLUMN IF NOT EXISTS actual_spend_cents integer;

-- 2) Ticket->CRM identity sync must respect scanned ticket name
...
-- 3) CRM stats: name-aware ticket mapping + split reservation spend per person
...
BEGIN
  ...
  ticket_identity AS (
    SELECT
      ...
      t.status,
      t.tier_id,
      COALESCE(name_match.id, user_name_match.id, user_fallback.id) AS g_id
    ...
    LEFT JOIN LATERAL (
      ...
      LIMIT 1
    ) user_name_match ON name_match.id IS NULL
    LEFT JOIN LATERAL (
      SELECT cg.id
      FROM public.crm_guests cg
      WHERE cg.business_id = p_business_id
        AND cg.user_id = t.user_id
        AND t.user_id IS NOT NULL
        AND public.normalize_guest_identity(t.guest_name) IS NULL
      LIMIT 1
    ) user_fallback ON name_match.id IS NULL AND user_name_match.id IS NULL
  ),
  ...
  profile_res_visits AS (
    SELECT
      COALESCE(name_match.id, user_match.id) AS g_id,
    ...
    LEFT JOIN LATERAL (
      ...
      LIMIT 1
    ) user_match ON name_match.id IS NULL
  ),
  event_res_visits AS (
    SELECT
      COALESCE(name_match.id, user_match.id) AS g_id,
  ...
  offer_spend AS (
    ...
    WHERE d.business_id = p_business_id
      AND op.status = 'paid'
    GROUP BY COALESCE(name_match.id, user_match.id)
  ),
  total_spend AS (
  ...
  fav_table AS (
    ...
    FROM (
      ...
      GROUP BY COALESCE(name_match.id, user_match.id), fpt.label
    ) sub

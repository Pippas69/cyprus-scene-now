...
AS $$
  ...
  -- 1. Ticket activities (ticket-only & hybrid events)
  ...
  -- 2. Reservation activities (non-hybrid: reservation-only events, profile, offer, walk-in)
  -- Booker match
  ...
  -- Ghost match for reservation events via reservation_guests
  ...
  -- Ghost match for profile/offer reservations (no individual entry, use booker's check-in)

...
-- Update search_content function to include offer boosts and proper priority hierarchy
CREATE OR REPLACE FUNCTION public.search_content(search_query text)
 RETURNS TABLE(result_type text, id uuid, name text, title text, logo_url text, cover_image_url text, city text, location text, start_at timestamp with time zone, category text[], business_name text, verified boolean, relevance_score integer, business_id uuid)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE
...
BEGIN
  ...
  WITH all_results AS (
    ...
    -- Priority: Boosted events (3) > Regular events (6)
    SELECT 
    ...
    -- Priority: Boosted offers (4) > Regular offers (7)
    SELECT 

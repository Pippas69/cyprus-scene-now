...
-- Ensure search_content only returns events/offers that are visible in the Events/Offers sections
-- (i.e., tied to verified businesses) and fix mutable search_path warning.
...
BEGIN
  ...
  RETURN QUERY
  WITH all_results AS (
    -- Businesses: keep current behavior (all businesses except suspended)
    SELECT
    ...
    -- Events: ONLY what the Events section can show
    -- (verified business + not expired)
    SELECT
    ...
    -- Offers: ONLY what the Offers section can show
    -- (verified business + active + in date window)
    SELECT

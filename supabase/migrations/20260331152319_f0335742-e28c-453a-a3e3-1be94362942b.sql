...
BEGIN
  ...
  WITH raw_visits AS (
    -- Tickets (registered users + manual entries)
    SELECT
      t.id AS record_id,
      t.user_id,
  ...
  demographics AS (
    SELECT
      CASE
        ...
        ELSE NULL
      END AS gender,
      -- ALWAYS prefer ticket_guest_age over profile age
      -- ticket_guest_age is the actual guest's age, profile.age might be the buyer/owner
      COALESCE(rv.ticket_guest_age, p.age) AS age,
      -- ALWAYS prefer ticket_guest_city over profile city
      COALESCE(

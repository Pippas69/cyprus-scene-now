...
AS $$
  ...
  -- 1. Ticket activities: only the ticket whose guest_name matches the CRM guest
  ticket_activities AS (
    ...
    WHERE COALESCE(t.status::text, '') <> 'cancelled'
      AND (
        -- Registered user: match by user_id AND guest_name must match
        ...
        -- Ghost: match by brought_by + name
        ...
        OR (gi.user_id IS NULL AND gi.brought_by_user_id IS NULL
            AND lower(trim(COALESCE(t.guest_name,''))) = lower(trim(COALESCE(gi.guest_name,''))))
        -- Manual entry: name match
        OR (COALESCE(t.is_manual_entry, false) = true
            AND lower(trim(COALESCE(t.guest_name,''))) = lower(trim(COALESCE(gi.guest_name,''))))
      )
  ),
  -- 2. Reservation activities (non-hybrid)
  ...
  -- Ghost in reservation events via reservation_guests

...
-- Update favorite_table to use actual floor plan table labels from reservation_table_assignments
-- instead of seating type names, and only for guests with visits > 0
...
BEGIN
  ...
  reservation_main_participant AS (
    ...
    WHERE r.checked_in_at IS NOT NULL
      ...
      AND NOT EXISTS (
        ...
        WHERE tord.reservation_id = r.id
      )
  ),
            ...
            GREATEST(
              ...
              COALESCE(
                (SELECT stt.min_spend_cents
                 ...
                 WHERE stt.seating_type_id = (SELECT r2.seating_type_id FROM public.reservations r2 WHERE r2.id = tord.reservation_id)
  ...
  -- SPEND: reservation-based (direct reservations without ticket_orders)
  ...
  -- Party stats & favorite table from FLOOR PLAN ASSIGNMENTS (actual table labels like 101, Π1, bar)
  -- Only counts visits where the guest was actually checked in (has a visit)

...
-- Fix: Manual entries must use prepaid_min_charge_cents as-is (what the business owner entered),
-- NOT the seating tier min_charge. Full amount goes to single ghost profile, regardless of check-in.
...
BEGIN
  ...
  -- ======== MANUAL ENTRY SPEND (simplified: full amount to single ghost) ========
  manual_spend AS (
    SELECT
      (SELECT cg.id FROM public.crm_guests cg WHERE cg.business_id = p_business_id AND public.normalize_guest_identity(cg.guest_name) = public.normalize_guest_identity(r.reservation_name) LIMIT 1) AS g_id,
      CASE
    ...
    LEFT JOIN public.events e ON e.id = r.event_id
    WHERE ((r.event_id IS NULL AND r.business_id = p_business_id) OR (r.event_id IS NOT NULL AND e.business_id = p_business_id))
      AND r.is_manual_entry = true
      AND r.status IN ('accepted', 'completed', 'no_show')
  ),

  manual_spend_agg AS (
    SELECT g_id, COALESCE(SUM(spend_cents), 0)::bigint AS spend_cents
    FROM manual_spend WHERE g_id IS NOT NULL GROUP BY g_id
  ),

  -- ======== 3-PHASE SPEND LOGIC (non-manual only) ========
  biz_reservations AS (
    SELECT
  ...
  total_spend AS (
    ...
    FROM (
      ...
      UNION ALL SELECT g_id, spend_cents FROM manual_spend_agg WHERE g_id IS NOT NULL
  ...
  -- Also include manual entry table assignments
  manual_table_data AS (
    SELECT
      (SELECT cg.id FROM public.crm_guests cg WHERE cg.business_id = p_business_id AND public.normalize_guest_identity(cg.guest_name) = public.normalize_guest_identity(r.reservation_name) LIMIT 1) AS g_id,
      fpt.label AS table_label
    FROM public.reservations r
    JOIN public.reservation_table_assignments rta ON rta.reservation_id = r.id
    JOIN public.floor_plan_tables fpt ON fpt.id = rta.table_id
    LEFT JOIN public.events e ON e.id = r.event_id
    WHERE r.is_manual_entry = true
      AND ((r.event_id IS NULL AND r.business_id = p_business_id) OR (r.event_id IS NOT NULL AND e.business_id = p_business_id))
  ),

  all_table_data AS (
    SELECT g_id, table_label FROM favorite_table_data WHERE g_id IS NOT NULL
    UNION ALL SELECT g_id, table_label FROM manual_table_data WHERE g_id IS NOT NULL
  ),

  favorite_table_stats AS (
    SELECT g_id, MODE() WITHIN GROUP (ORDER BY table_label) AS favorite_table
    FROM all_table_data WHERE g_id IS NOT NULL GROUP BY g_id
  ),

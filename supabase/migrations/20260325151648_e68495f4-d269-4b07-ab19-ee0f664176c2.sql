...
-- =============================================================
-- UNIFIED VISIT DEDUPLICATION & USER-FIRST IDENTITY MATCHING
-- =============================================================
-- Fixes:
-- 1. ticket_visits: COALESCE(name_match, user_match) → COALESCE(user_match, name_match)
-- 2. ticket_spend: same fix
-- 3. all_visits: deduplicate ticket+reservation for same user+event
-- 4. get_audience_demographics: deduplicate same user+event

-- =============================================
-- Fix get_crm_guest_stats
-- =============================================
...
BEGIN
  ...
  -- FIX: Use COALESCE(user_match, name_match) — user-first matching
  ticket_visits AS (
    SELECT
    ...
    LEFT JOIN LATERAL (
      ...
      ORDER BY CASE WHEN cg.user_id IS NOT NULL THEN 0 ELSE 1 END, cg.created_at
      LIMIT 1
  ...
  -- DEDUP: For event-related visits, if same user has both ticket+reservation
  -- for the same event, count only once (keep the earliest visited_at)
  raw_visits AS (
  ...
  -- Deduplicate: same guest + same event = 1 visit
  -- Non-event visits (visit_event_id IS NULL) are always unique
  all_visits AS (
  ...
  -- FIX: ticket_spend also user-first
  ticket_spend AS (
    SELECT
      COALESCE(user_match.id, name_match.id) AS g_id,
...
-- =============================================
-- Fix get_audience_demographics - add dedup
-- =============================================
...
BEGIN
  ...
  WITH raw_visits AS (
    -- 1) Ticket check-ins (with event_id for dedup)
    ...
    -- 2) Offer redemptions (no event, always unique)
  ...
  -- DEDUP: same user + same event = 1 visit
  -- Non-event visits get unique IDs so they're never deduped
  deduped_visits AS (

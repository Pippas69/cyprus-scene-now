-- Fix get_audience_demographics to exclude offer-marked reservations
-- Exclude reservations marked with "Offer claim:" in special_requests
-- (they are already counted via offer_purchases.redeemed_at)
...
BEGIN
  ...
  WITH all_visits AS (
    ...
    -- 3) Direct profile reservation check-ins
    -- IMPORTANT: Exclude reservations that were created via offers
    -- - Either via offer_purchases.reservation_id
    -- - Or marked with "Offer claim:" in special_requests
    -- Those are already counted in section 2 above.
    ...
    WHERE (r.business_id = p_business_id OR e.business_id = p_business_id)
      ...
      -- Exclude offer-linked reservations to prevent double-counting
      AND NOT EXISTS (
        SELECT 1 FROM offer_purchases op2
        WHERE op2.reservation_id = r.id
      )
      -- Also exclude reservations marked as offer claims
  ...
  demographics AS (
    SELECT
      ...
      p.gender,
      p.age,
      -- FIX: Use NULLIF to treat empty strings as NULL, then fallback to town

-- Phase 1: Add composite indexes for performance
CREATE INDEX IF NOT EXISTS idx_event_views_event_date ON event_views(event_id, viewed_at DESC);
CREATE INDEX IF NOT EXISTS idx_discount_views_discount_date ON discount_views(discount_id, viewed_at DESC);
CREATE INDEX IF NOT EXISTS idx_engagement_business_date ON engagement_events(business_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_rsvps_event_created ON rsvps(event_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_reservations_event_created ON reservations(event_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_business_followers_business_date ON business_followers(business_id, created_at DESC);

-- Phase 2: Create optimized analytics aggregation function

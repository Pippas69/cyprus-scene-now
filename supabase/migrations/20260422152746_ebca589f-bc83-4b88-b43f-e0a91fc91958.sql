-- Version 1: book_slot_atomically (9 args, no p_source)
CREATE OR REPLACE FUNCTION public.book_slot_atomically(
    ...
    p_seating_preference text DEFAULT NULL::text,
...
-- Version 2: book_slot_atomically (10 args, with p_source)
CREATE OR REPLACE FUNCTION public.book_slot_atomically(
    p_business_id uuid,
    p_date date,
    p_slot_time text,
    ...
    p_special_requests text DEFAULT NULL::text,
    p_is_offer_based boolean DEFAULT false,
    p_source text DEFAULT 'profile'::text
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER

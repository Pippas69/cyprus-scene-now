...
BEGIN
  ...
  -- KEY FIX: Phase 2 activates when checked_in_count > 0, not only when event ended
  participant_spend AS (
    SELECT
      arp.g_id,
      CASE
        -- Phase 1: No check-ins yet AND event not ended → prepayment / party_size
        WHEN NOT rc.event_ended AND rc.checked_in_count = 0 THEN rc.prepay_pp
        -- No-show: locked at prepayment / party_size (regardless of phase)

...
-- Hybrid spend: distribute reservation spend to ALL ghosts in the party via ticket_mapped
...
-- Reservation-only spend: per-person distribution to booker + all ghosts
-- Manual entries: full amount to matched CRM guest (no party division)
...
-- Manual entries: full amount to the matched guest (no division)
...
-- Non-manual: per-person amount distributed to booker + all ghosts
...
reservation_spend_distributed AS (
  ...
  UNION ALL
  -- Ghosts get the same per-person amount
  SELECT cg.id AS g_id, rspp.spend_per_person AS spend_cents
  FROM reservation_spend_per_person rspp
...
party_size_agg AS (
  SELECT g_id, ROUND(AVG(party_size), 1) AS avg_party_size FROM party_size_stats WHERE g_id IS NOT NULL GROUP BY g_id
),
reservation_count AS (
  ...
  FROM public.reservations r
  LEFT JOIN public.events e ON e.id = r.event_id
  WHERE r.status IN ('accepted', 'completed', 'pending') AND ((r.event_id IS NULL AND r.business_id = p_business_id) OR (r.event_id IS NOT NULL AND e.business_id = p_business_id))
),
reservation_count_agg AS (
  SELECT g_id, COUNT(*)::bigint AS total_reservations FROM reservation_count WHERE g_id IS NOT NULL GROUP BY g_id
),
ticket_demographics AS (
  SELECT DISTINCT ON (tm.g_id) tm.g_id,
    t.guest_age,
    NULLIF(trim(COALESCE(t.guest_city, '')), '') AS guest_city
  FROM ticket_mapped tm
  JOIN public.tickets t ON t.id = tm.ticket_id
  WHERE tm.g_id IS NOT NULL AND (t.guest_age IS NOT NULL OR NULLIF(trim(COALESCE(t.guest_city, '')), '') IS NOT NULL)
  ORDER BY tm.g_id, t.created_at DESC
),
ticket_demographics_fallback AS (
  SELECT DISTINCT ON (cg.id) cg.id AS g_id,
    t.guest_age,
    NULLIF(trim(COALESCE(t.guest_city, '')), '') AS guest_city
  FROM public.crm_guests cg
  JOIN public.tickets t ON lower(trim(t.guest_name)) = lower(trim(cg.guest_name))
  JOIN public.events e ON e.id = t.event_id AND e.business_id = p_business_id
  WHERE cg.business_id = p_business_id
    AND (t.guest_age IS NOT NULL OR NULLIF(trim(COALESCE(t.guest_city, '')), '') IS NOT NULL)

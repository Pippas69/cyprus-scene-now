...
-- Add override columns for manual editing from CRM profile
ALTER TABLE public.crm_guests ADD COLUMN IF NOT EXISTS spend_override_cents bigint DEFAULT NULL;
ALTER TABLE public.crm_guests ADD COLUMN IF NOT EXISTS favorite_table_override text DEFAULT NULL;

-- Rebuild get_crm_guest_stats with:
-- 1) Standalone ticket spend + offer spend (missing from current)
-- 2) Override support for spend and favorite_table
-- 3) Correct 3-phase reservation spend logic
...
BEGIN
  ...
  -- Resolve participants for each reservation
  ...
  -- Compute formula values per reservation
  res_formula AS (
    SELECT
      rwt.res_id,
      rwt.party_size,
  ...
  -- 3-phase per-participant spend
  participant_spend AS (
    SELECT
      arp.g_id,
      CASE
        -- Phase 1: event not ended → prepayment / party_size
  ...
  -- ======== STANDALONE TICKET SPEND (tickets NOT linked to a reservation) ========
  standalone_ticket_spend AS (
    ...
    WHERE t.status::text IN ('valid', 'used')
      -- Exclude tickets that belong to orders linked to reservations (already counted in reservation spend)
      AND NOT EXISTS (
  ...
  -- ======== TOTAL SPEND (all sources combined) ========
  total_spend AS (
  ...
  SELECT
    ...
    -- Use spend_override if set, otherwise computed total
    ...
    -- Use favorite_table_override if set, otherwise computed

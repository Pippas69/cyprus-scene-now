-- Align audience demographics to the same "verified visits" definition used across Analytics (Overview/Performance)
-- Sources:
-- 1) Ticket check-ins (tickets.checked_in_at) for events owned by the business
-- 2) Offer redemptions (offer_purchases.redeemed_at) for offers owned by the business
-- 3) Reservation check-ins (reservations.checked_in_at) for direct reservations OR event reservations for events owned by the business

...
BEGIN
  ...
  -- Detect hybrid reservations (those with linked ticket orders)
  reservations_with_tickets AS (
    SELECT DISTINCT o.linked_reservation_id AS reservation_id
    FROM public.ticket_orders o
    WHERE o.linked_reservation_id IS NOT NULL
  ),

  -- Main booker: ONLY for non-hybrid reservations
  -- In hybrid events, all participants are tracked individually via tickets
  reservation_main_participant_ext AS (
    ...
    WHERE NOT EXISTS (
      SELECT 1 FROM reservations_with_tickets rwt WHERE rwt.reservation_id = rsb.reservation_id
    )
  ),
  ...
  total_no_shows AS (
    ...
    FROM (
      SELECT g_id, no_shows FROM ticket_no_shows WHERE g_id IS NOT NULL
      UNION ALL SELECT g_id, no_shows FROM event_guest_no_shows WHERE g_id IS NOT NULL
  ...
  reservation_party AS (
    ...
    FROM public.reservations r
    LEFT JOIN public.events e ON e.id = r.event_id
    LEFT JOIN public.seating_types st ON st.id = r.seating_type_id
    LEFT JOIN LATERAL (
      SELECT cg.id
      FROM public.crm_guests cg
      WHERE cg.business_id = p_business_id
  ...
  party_stats AS (
    SELECT
      ...
      MODE() WITHIN GROUP (ORDER BY table_name) AS favorite_table
  ...
  LEFT JOIN total_no_shows tns ON tns.g_id = cg.id

...
ticket_exact_index AS (
  SELECT str.ticket_id,
    row_number() OVER (PARTITION BY COALESCE(str.buyer_id, str.original_user_id), str.guest_name_exact ORDER BY str.created_at, str.ticket_id) AS seq_no
...
ticket_mapped AS (
  SELECT str.ticket_id, str.event_id, str.checked_in_at, str.status_text, str.price_cents, str.manual_status,
...
ticket_no_shows AS (
  ...
  WHERE tm.g_id IS NOT NULL AND tm.checked_in_at IS NULL
    AND (
      (be.end_at + interval '10 hours') < now()
      OR tm.manual_status = 'no_show'
    )
  GROUP BY tm.g_id
),

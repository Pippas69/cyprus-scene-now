...
DROP FUNCTION IF EXISTS public.get_crm_guest_stats_v2(uuid);
...
standalone_ticket_rows AS (
  SELECT
    t.id AS ticket_id, t.event_id, t.checked_in_at,
    ...
    COALESCE(
      tord.subtotal_cents / GREATEST(
        (SELECT COUNT(*) FROM public.tickets t2 WHERE t2.order_id = t.order_id AND COALESCE(t2.status::text, '') <> 'cancelled'), 1
      ),
...
ticket_exact_index AS (
  SELECT str.ticket_id,
    row_number() OVER (PARTITION BY str.buyer_id, str.guest_name_exact ORDER BY str.created_at, str.ticket_id) AS seq_no
  FROM standalone_ticket_rows str WHERE str.guest_name_exact IS NOT NULL
),
ghost_exact_index AS (
  SELECT cg.id AS guest_id, cg.brought_by_user_id AS buyer_id,
    NULLIF(trim(COALESCE(cg.guest_name, '')), '') AS guest_name_exact,
    row_number() OVER (PARTITION BY cg.brought_by_user_id, NULLIF(trim(COALESCE(cg.guest_name, '')), '') ORDER BY cg.created_at, cg.id) AS seq_no
  FROM public.crm_guests cg
  WHERE cg.business_id = p_business_id AND cg.user_id IS NULL AND cg.profile_type IN ('ghost', 'merged')
),
ticket_mapped AS (
  SELECT str.ticket_id, str.event_id, str.checked_in_at, str.status_text, str.price_cents,
    COALESCE(gei.guest_id, public.resolve_crm_guest_for_ticket(p_business_id, str.buyer_id, str.guest_name_exact)) AS g_id
  FROM standalone_ticket_rows str
  LEFT JOIN ticket_exact_index tei ON tei.ticket_id = str.ticket_id
  LEFT JOIN ghost_exact_index gei ON gei.buyer_id IS NOT DISTINCT FROM str.buyer_id AND gei.guest_name_exact = str.guest_name_exact AND gei.seq_no = tei.seq_no
),
ticket_visits AS (
  SELECT tm.g_id, MIN(tm.checked_in_at) AS visited_at, tm.event_id
  FROM ticket_mapped tm WHERE tm.g_id IS NOT NULL AND tm.checked_in_at IS NOT NULL
...
ticket_no_shows AS (
  ...
  WHERE tm.g_id IS NOT NULL AND tm.checked_in_at IS NULL AND be.end_at < now()
  GROUP BY tm.g_id
),
...
ticket_demographics AS (
  SELECT DISTINCT ON (tm.g_id)
    tm.g_id, t.guest_age, t.guest_city
...
SELECT
  ...
  COALESCE(td.guest_age, (SELECT p.age FROM public.profiles p WHERE p.id = cg.user_id))::integer AS guest_age,
  COALESCE(td.guest_city, (SELECT COALESCE(p.city, p.town) FROM public.profiles p WHERE p.id = cg.user_id))::text AS guest_city
FROM public.crm_guests cg
...
CREATE OR REPLACE FUNCTION public.get_crm_guest_stats(p_business_id uuid)
 ...
 SET search_path TO 'public'
AS $function$
  SELECT * FROM public.get_crm_guest_stats_v2(p_business_id);
$function$;

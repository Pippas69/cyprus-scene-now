-- Fix ambiguity with RETURNS TABLE column names inside PL/pgSQL
...
BEGIN
  ...
  WITH base AS (
    ...
    SELECT
      ...
      -- sorting helpers
      (eb.id IS NOT NULL)::boolean AS is_boosted,
      99::integer AS tier_rank,
      CASE WHEN eb.id IS NOT NULL THEN 0 ELSE 2 END AS type_rank
    FROM events e
    ...
    SELECT
      ...
      -- sorting helpers
      (ob.id IS NOT NULL)::boolean AS is_boosted,
      99::integer AS tier_rank,
      CASE WHEN ob.id IS NOT NULL THEN 0 ELSE 2 END AS type_rank
    FROM discounts d

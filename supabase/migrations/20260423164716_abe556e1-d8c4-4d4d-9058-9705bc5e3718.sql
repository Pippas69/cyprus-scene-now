...
BEGIN
  LOOP
    ...
    FOR i IN 0..15 LOOP
      new_token := new_token || substr(alphabet, (get_byte(rand_bytes, i) % length(alphabet)) + 1, 1);
    END LOOP;
    IF NOT EXISTS (SELECT 1 FROM public.pending_bookings pb WHERE pb.token = new_token) THEN
      RETURN new_token;
    END IF;
    attempts := attempts + 1;
    IF attempts > 10 THEN

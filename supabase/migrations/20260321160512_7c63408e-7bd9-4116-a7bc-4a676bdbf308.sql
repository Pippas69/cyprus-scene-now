...
BEGIN
  ...
  all_visits AS (
    SELECT g_id, visited_at FROM ticket_visits WHERE g_id IS NOT NULL
    UNION ALL
    SELECT g_id, visited_at FROM profile_res_visits WHERE g_id IS NOT NULL
    UNION ALL
    SELECT g_id, visited_at FROM event_res_visits WHERE g_id IS NOT NULL
    UNION ALL
    SELECT g_id, visited_at FROM offer_visits WHERE g_id IS NOT NULL
    UNION ALL
    SELECT g_id, visited_at FROM student_visits WHERE g_id IS NOT NULL
  ),
  visit_stats AS (
    SELECT
      v.g_id,

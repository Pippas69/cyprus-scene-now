...
-- Fix CRM stats for manual entries: when is_manual_entry = true,
-- user_id is the business owner, NOT the guest. Must match by name only.
...
BEGIN
  ...
  ticket_visits AS (
    SELECT
      -- For manual ticket entries, ignore user_id (it's the business owner)
  ...
  booker_direct_visits AS (
    SELECT
      -- For manual entries, match ONLY by name (user_id is the business owner)

...

  {
    ...
    try {
      ...
      if (legacyReservationId) {
        ...
        let pbNotesLegacy: string | null = null;
        if (metadata?.pending_booking_id || metadata?.pending_booking_token) {
          ...
          const rawNotes = (pbRowLegacy as { notes?: string | null } | null)?.notes;
          pbNotesLegacy = rawNotes && String(rawNotes).trim() ? String(rawNotes).trim() : null;
        }
        ...
        // Only overwrite special_requests if pb.notes exists AND there's no client-supplied value
        if (pbNotesLegacy && !specialRequests) legacyUpdate.special_requests = pbNotesLegacy;
        ...
        // Φάση 4: If linked to a pending_booking, fetch its care_of + notes so we can persist them on the reservation
        let pbCareOf: string | null = null;
        let pbNotes: string | null = null;
        if (metadata?.pending_booking_id || metadata?.pending_booking_token) {
          ...
          const rawNotes = (pbRow as { notes?: string | null } | null)?.notes;
          pbNotes = rawNotes && String(rawNotes).trim() ? String(rawNotes).trim() : null;
        }

        // Use client-supplied special_requests when provided; otherwise fall back to pb.notes
        const finalSpecialRequests = specialRequests || pbNotes;

...
//   4. Check SMS rate limits BEFORE creating any DB rows
//   5. Insert pending_booking (DB trigger generates 16-char token)
//   6. Send SMS via send-booking-sms function
//   7. If SMS fails → mark pending_booking as 'cancelled' (cleanup)
//   8. Return { token, booking_url, pending_booking_id }
...
// E.164 phone validation (must include +, 8-15 digits)
const e164Phone = z
  .string()
  .trim()
...
function serve_handler() {
  Deno.serve(async (req: Request) => {
    ...
    try {
      ...
      // ── 5. Pre-check SMS rate limit (avoid creating orphan booking) ──
      ...
      const allowed = (rateCheck as { allowed?: boolean } | null)?.allowed;
      const reason = (rateCheck as { reason?: string } | null)?.reason;
      if (!allowed) {
        return jsonResponse(
          { error: "Rate limit exceeded", reason: reason ?? "rate_limited" },
          429
        );
      }
...
// Re-export Deno.serve as named for clarity
function serve(_handler: (req: Request) => Promise<Response>) {
  // no-op — using Deno.serve directly above
}
